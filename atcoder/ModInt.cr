...
module AtCoder
  record ModInt, value : Int64 do
    @@factorials = Array(ModInt).new(100_000_i64) # Change here to improve performance

    def self.factorial(n)
      if @@factorials.empty?
        @@factorials << ModInt.new(1_i64)
      end
    ...
    def inv
      g, x = ModInt.extended_gcd(@value, MOD)
      ModInt.new(x % MOD)
    end

    def +(value)
      ModInt.new((@value + value.to_i64 % MOD) % MOD)
    end

    def -(value)
      ModInt.new((@value + MOD - value.to_i64 % MOD) % MOD)
    end
    ...
    def /(value)
      raise DivisionByZeroError.new if value == 0
      self * ModInt.new(value.to_i64 % MOD).inv
    end
