...
if [ "$filename" -nt "${output}" ]; then
  ...
  elif [ -d /usr/local/share/crystal-1.8.2 ]; then
    /usr/local/share/crystal-1.8.2/bin/crystal build "$filename" -o "${output}" --error-trace $@ || exit 1
  elif [ -d /tmp/crystal-1.8.2 ]; then
    /tmp/crystal-1.8.2/bin/crystal build "$filename" -o "${output}" --error-trace $@ || exit 1
  elif [[ "`crystal version`" == *"1.8.2"* ]]; then
    crystal build "$filename" -o "${output}" --error-trace $@ || exit 1
  else
