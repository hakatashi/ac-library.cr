...
if [ "$filename" -nt "${output}" ]; then
  ...
  if [ -d /usr/local/share/crystal-0.33.0-1 ]; then
    /usr/local/share/crystal-0.33.0-1/bin/crystal build "$filename" -o "${output}" --release --error-trace $@ || exit 1
  elif [ -d /tmp/crystal-0.33.0-1 ]; then
    /tmp/crystal-0.33.0-1/bin/crystal build "$filename" -o "${output}" --release --error-trace $@ || exit 1
  elif [[ "`crystal version`" == *"0.33.0"* ]]; then
    crystal build "$filename" -o "${output}" --release --error-trace $@ || exit 1
  else
